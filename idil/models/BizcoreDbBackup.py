...
class BizcoreDbBackupWizard(models.TransientModel):
    ...
    def _get_oci_config(self):
        ...
        if missing:
            raise UserError(
                _(
                    "Missing OCI configuration in odoo.conf: %s\n\n"
                    "Please add these values to your odoo.conf file."
                )
                % ", ".join(missing)
            )
    ...
    def _get_oci_client(self):
        ...
        if not OCI_AVAILABLE:
            raise UserError(
                _(
                    "OCI SDK not installed. Please install it on the server:\n"
                    "pip3 install oci"
                )
            )
    ...
    def _list_oci_backups(self):
        ...
        list_objects = client.list_objects(
            namespace_name=namespace, bucket_name=bucket, prefix="backup_"
        )
    ...
    def _generate_oci_download_url(self, object_name, expiration_hours=24):
        ...
        region = config.get("oci_region")
        full_url = (
            f"https://objectstorage.{region}.oraclecloud.com{par.data.access_uri}"
        )
            ...
            try:
                ...
                return {
                    ...
                    "params": {
                        ...
                        "message": _(
                            "Backup uploaded to OCI Object Storage. Download link generated (valid 24h)."
                        ),
                        "type": "success",
                        "sticky": True,
                        "next": {
    ...
    # Replace the action_restore_to_new_db method with this improved version:
    ...
    def action_restore_to_new_db(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                if result.returncode != 0:
                    ...
                    raise UserError(_("Database restore failed: %s") % error_msg)

            except subprocess.TimeoutExpired:
                raise UserError(_("Database restore timed out after 5 minutes"))
            except subprocess.CalledProcessError as e:
                raise UserError(_("Database restore failed: %s") % str(e))
            except Exception as e:
                raise UserError(_("Unexpected error during restore: %s") % str(e))
        ...
        # Set result BEFORE any database operations that might fail
        restore_result = (
        ...
        # Use a new cursor to avoid connection issues
        try:
            with self.env.cr.savepoint():
                self.write({"restore_result": restore_result})
        except Exception as e:
            _logger.warning("Could not update restore result: %s", e)

        # Return immediately to avoid further database operations
        return {
            "type": "ir.actions.client",
            "tag": "display_notification",
            "params": {
