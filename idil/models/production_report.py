...
class IdilProductionReportWizard(models.TransientModel):
    ...
    order_ids = fields.Many2many('idil.manufacturing.order', string='Included MOs', readonly=True)
    ...
    def _get_domain(self):
        ...
        statuses = []
        if self.status_draft: statuses.append('draft')
        if self.status_confirmed: statuses.append('confirmed')
        if self.status_in_progress: statuses.append('in_progress')
        if self.status_done: statuses.append('done')
    ...
    def _compute_orders(self):
        for rec in self:
            ...
            rec.order_ids = self.env['idil.manufacturing.order'].search(domain, order='scheduled_start_date desc')
    ...
    def _compute_kpi_html(self):
        for rec in self:
            ...
            total_production_cost_sos = bom_sos_total + extra_sos_total + commission_sos_total
            avg_unit_cost_sos = total_production_cost_sos / total_produced_qty if total_produced_qty else 0
    ...
    def _compute_insights(self):
        for rec in self:
            ...
            last_week_orders = self.env['idil.manufacturing.order'].search(last_week_domain)
            ...
            current_avg_cost = current_total_cost / current_total_qty if current_total_qty else 0
            ...
            if past_avg_cost > 0 and current_avg_cost > past_avg_cost * 1.10:
                insights.append(f"⚠️ <b>Unit Cost Spike:</b> Current unit cost (SOS {current_avg_cost:,.0f}) is >10% higher than last week's average (SOS {past_avg_cost:,.0f}). Investigate extra costs or material usage.")
            ...
            if total_bom_sos > 0 and total_extra_sos > (total_bom_sos * 0.20):
                 insights.append("⚠️ <b>High Extra Costs:</b> Extra costs exceed 20% of BOM costs.")
                 
            total_comm = sum(orders.mapped('commission_amount'))
            if current_total_cost > 0 and total_comm > (current_total_cost * 0.15):
                 insights.append("⚠️ <b>High Commissions:</b> Commissions are over 15% of production cost.")

            missing_emp_cost_lines = self.env['idil.manufacturing.order.cost.line'].search_count([
                ('manufacturing_order_id', 'in', orders.ids),
                ('employee_id', '=', False)
            ])
            if missing_emp_cost_lines > 0:
                 insights.append(f"❌ <b>Missing Cost Ownership:</b> {missing_emp_cost_lines} extra cost lines have no employee assigned.")
            
            if not insights:
                insights.append("✅ <b>Production looks stable.</b> No major anomalies detected.")
                
            rec.insights_html = "<ul class='list-unstyled'>" + "".join([f"<li class='mb-2'>{i}</li>" for i in insights]) + "</ul>"

    def _compute_commission_html(self):
        for rec in self:
            orders = rec.order_ids
            if not orders:
                rec.commission_html = "<div class='alert alert-warning'>No data available</div>"
                continue
            ...
            prod_map = {p.id: p.name for p in self.env['my_product.product'].browse(prod_ids)}
            ...
            self.env.cr.execute("SELECT SUM(commission_amount) FROM idil_manufacturing_order WHERE id IN %s", (order_ids_tuple,))
            ...
            avg_comm_unit = total_comm_real / total_qty_produced if total_qty_produced else 0
            
            # Production Cost for % Calc
            total_production_cost = sum(o.report_total_sos for o in orders)
            comm_pct_cost = (total_comm_real / total_production_cost * 100) if total_production_cost else 0
            
            count_employees = len(emp_ids)
            highest_earner_name = emp_map.get(emp_data_raw[0][0], "N/A") if emp_data_raw else "None"
            highest_earner_val = emp_data_raw[0][1] if emp_data_raw else 0
            
            lowest_earner_name = emp_map.get(emp_data_raw[-1][0], "N/A") if emp_data_raw else "None"
            ...
            insights_list = []
            if comm_pct_cost > 15:
                insights_list.append(f"⚠️ <b>High Ratio:</b> Commission is {comm_pct_cost:.1f}% of production cost (Target: <15%).")
            if top_3_share > 70:
                insights_list.append(f"⚠️ <b>Concentration Risk:</b> Top 3 employees take {top_3_share:.0f}% of all commissions.")
            
            # Check for missing employees
            self.env.cr.execute("SELECT COUNT(*) FROM idil_manufacturing_order WHERE id IN %s AND commission_amount > 0 AND commission_employee_id IS NULL", (order_ids_tuple,))
            missing_emp_count = self.env.cr.fetchone()[0]
            if missing_emp_count > 0:
                insights_list.append(f"❌ <b>Missing Assignment:</b> {missing_emp_count} MOs have commission amount but no employee.")
            ...
            self.env.cr.execute("""
                ...
                AND company_id = %s
            """, (datetime.combine(start_prev, time.min), datetime.combine(end_prev, time.max), rec.company_id.id))
        ...
        for rec in self:
            ...
            for mo in orders:
                ...
                if pid not in products:
                    products[pid] = {'name': mo.product_id.name, 'qty': 0, 'tfg_sum': 0, 'min_tfg': 999999, 'max_tfg': 0}
                
                products[pid]['qty'] += mo.product_qty
                tfg = mo.tfg_qty or 0
                products[pid]['tfg_sum'] += (tfg * mo.product_qty)
                
                if tfg < products[pid]['min_tfg']: products[pid]['min_tfg'] = tfg
                if tfg > products[pid]['max_tfg']: products[pid]['max_tfg'] = tfg
            ...
            for pid, vals in products.items():
                if vals['min_tfg'] == 999999: vals['min_tfg'] = 0
    ...
    def _compute_accounting_html(self):
        for rec in self:
            ...
            bookings = self.env['idil.transaction_booking'].search([('manufacturing_order_id', 'in', rec.order_ids.ids)])
            
            # Find lines related to those bookings
            lines = self.env['idil.transaction_bookingline'].search([('transaction_booking_id', 'in', bookings.ids)])
    ...
    def _get_comparison_row(self, label, curr, prev, is_cost=False):
        ...
        if prev != 0:
            if is_cost:
                if diff > 0: color = "text-danger" # Cost up = bad
                elif diff < 0: color = "text-success" # Cost down = good
            else: # Qty or other 'good' metrics
                 if diff > 0: color = "text-success" # Qty up = good
                 elif diff < 0: color = "text-danger" # Qty down = bad
    ...
    def _compute_graphs_html(self):
        for rec in self:
            orders = rec.order_ids
            rec.extra_kpi_html = "" # Clear this, we will use graphs_html for the whole dashboard
            ...
            def render_svg(chart_type, data, title, height=220, color="#4e73df", stacked=False):
                if not data: return f'<div class="alert alert-light">No data for {title}</div>'
                ...
                # Determine Max Value
                if stacked:
                    max_val = max((d.get('v1',0) + d.get('v2',0) + d.get('v3',0)) for d in data) or 1
                else:
                ...
                step_x = plot_w / (count - 1) if count > 1 and chart_type == 'line' else plot_w / count
                
                # Draw Chart
                if chart_type == 'bar':
                    for i, d in enumerate(data):
                        x = pad_left + (i * (plot_w / count)) + ((plot_w / count - bar_w) / 2)
                    ...
                    # Close Area
                    poly_points.append(f"{pad_left + ((count-1)*step_x)},{pad_top+plot_h}")
                ...
                for i, d in enumerate(data):
                    if i % step_skip == 0:
                        x_pos = pad_left + (i * (plot_w/count)) + (plot_w/count/2) if chart_type != 'line' else pad_left + (i * step_x)
                        
                        # Fix for last label cutoff in line chart
                        if chart_type == 'line' and i == count-1: x_pos -= 10
                ...
                <div class="row mb-4">
                    <div class="col-md-6">
                        <div class="db-card">
                            ...
                            <div class="db-body">
            """
            c1_data = [{'label': d[0].strftime('%d-%b'), 'value': d[1]} for d in daily_data]
            html += render_svg('bar', c1_data, "Daily Produced Qty", height=240, color="#4e73df")
            html += """     </div>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="db-card">
                            ...
                            <div class="db-body">
            """
            c2_data = [{'label': d[0].strftime('%d-%b'), 'value': d[2]+d[3]+d[4]} for d in daily_data]
            html += render_svg('line', c2_data, "Daily Total Cost", height=240, color="#1cc88a")
            html += """     </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="row mb-4">
                     <div class="col-md-6">
                        <div class="db-card">
                            ...
                            <div class="db-body">
            """
            c3_data = [{'label': d[0].strftime('%d-%b'), 'value': ((d[2]+d[3]+d[4])/d[1] if d[1] else 0)} for d in daily_data]
            html += render_svg('line', c3_data, "Avg Unit Cost", height=240, color="#f6c23e")
            html += """     </div>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="db-card">
                            ...
                            <div class="db-body">
            """
            c4_data = [{'label': d[0].strftime('%d-%b'), 'v1': d[2], 'v2': d[3], 'v3': d[4]} for d in daily_data]
            html += render_svg('bar', c4_data, "Cost Breakdown", height=240, stacked=True)
            html += """
                         ...
                         <div class="db-card">
                            ...
                            <div class="p-0 table-responsive">
                                <table class="table table-sm table-striped mb-0 text-dark">
                                    ...
                                    <tbody>
            """
            mo_list = sorted(orders, key=lambda x: (x.report_total_sos/x.product_qty) if x.product_qty else 0, reverse=True)[:8]
            avg_network = avg_unit_cost
            for m in mo_list:
                u_c = (m.report_total_sos/m.product_qty) if m.product_qty else 0
                is_spike = u_c > avg_network * 1.10
                spike_html = ' <span class="badge badge-danger">Spike</span>' if is_spike else ''
            ...
            prod_sorted = sorted(prod_data, key=lambda x: (x[2]/x[1]) if x[1] else 0, reverse=True)[:8]
            for p in prod_sorted:
                p_name = p[0]
                # specific check for jsonb translation dict from raw sql
                if isinstance(p_name, dict):
                    p_name = p_name.get('en_US') or list(p_name.values())[0] if p_name else 'Unknown'
            ...
            """
            spike_count = sum(1 for m in mo_list if ((m.report_total_sos/m.product_qty) if m.product_qty else 0) > avg_network * 1.10)
            if spike_count > 0:
    ...
    def action_view_daily_summary(self):
        self.ensure_one()
        action = self.env["ir.actions.actions"]._for_xml_id("idil.action_production_report_daily_view")
        action['domain'] = self._get_view_domain()
        return action

    def action_view_product_breakdown(self):
        self.ensure_one()
        action = self.env["ir.actions.actions"]._for_xml_id("idil.action_production_report_product_view")
        action['domain'] = self._get_view_domain()
        return action
...
class IdilProductionReportView(models.Model):
    ...
    manufacturing_order_id = fields.Many2one('idil.manufacturing.order', string="MO", readonly=True)
