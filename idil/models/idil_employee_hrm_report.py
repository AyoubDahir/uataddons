...
class HRMSalaryReportWizard(models.TransientModel):
    _name = 'report.hrm.salary.report'
    _description = 'HRM Salary Report Wizard'

    start_date = fields.Date(string="Start Date", required=True)
    end_date = fields.Date(string="End Date", required=True)

    def generate_salary_report_pdf(self, export_type="pdf"):
        ...
        query = f"""                 
            select e.staff_id, 
                  ...
                  e.bonus,
                  es.deductions,
                  es.advance_deduction as advance_salary,
                  es.total_salary as net_salary
            from idil_employee e
            inner join idil_employee_position ep
        ...
        report_data = [
            {
                ...
                'deductions': row[5] if row[5] is not None else 0.00,
                'advance': row[6] if row[6] is not None else 0.00,
                'net_salary': row[7] if row[7] is not None else 0.00,
            } for row in results
        ]
        ...
        if export_type == "pdf":
            ...
            title = Paragraph("<b>HRM Salary Report</b>", styles['Title'])
